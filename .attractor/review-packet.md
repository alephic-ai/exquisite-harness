...
Ticket AI-3129: eh run: emit gateway-rate costUsd on headless usage events

- AC1: `eh run` computes cost from its own cumulative normalized usage x the resolved model's gateway rates — honoring `--gateway-provider` pins (per-endpoint pricing, not model-aggregate) and tiered prompt/completion rates — and emits it on the final `cumulative: true` `usage` event and/or `run.completed` as `costUsd` with `costSource: "gateway-rates"`.
- AC2: A harness-reported cost, when present, is preserved in a separate field (e.g. `harnessCostUsd`), never silently preferred over the computed one.
- AC3: When rates are unavailable (ollama, unknown model), no fabricated cost is emitted and `costSource` says why.
- AC4: Cache read/write tokens are billed at the endpoint's published cache rates; when an endpoint publishes no cache rates, the chosen behavior (bill as regular input vs zero) is explicit and documented in the code.
- AC5: If the event shape changes, the NDJSON event `v` field is bumped per eh's versioned-stream convention.
...
index 95e87c7..0c2c1b5 100644
--- a/src/headless-run.test.ts
+++ b/src/headless-run.test.ts
@@ -73,17 +73,17 @@ describe('eh run', () => {
       model: 'qwen3-coder',
       provider: 'ollama',
       type: 'run.started',
-      v: 1,
...
+      v: 2,
     ...
     const fakeEvent = z
       .object({
@@ -951,18 +1095,18 @@ describe('eh run', () => {
         model: 'test-model',
         provider: 'test-gateway',
         type: 'run.started',
-        v: 1,
...
@@ -1075,17 +1219,33 @@ describe('eh run', () => {
       inputTokens: 5,
       outputTokens: 2,
       type: 'usage',
-      v: 1,
...
+      v: 2,
     ...
     const argsEvent = events
@@ -1217,22 +1377,22 @@ describe('eh run', () => {
     expect(events).toContainEqual({
...
+      v: 2,
     ...
     const argsEvent = events
@@ -1295,13 +1455,13 @@ describe('eh run', () => {
       expect(events).toContainEqual({
...
@@ -1374,13 +1534,13 @@ describe('eh run', () => {
     expect(events).toContainEqual({
...
@@ -62,6 +65,25 @@ interface ResolvedHeadlessRunOptions {
...
@@ -154,6 +184,11 @@ export async function runHeadless(options: HeadlessRunOptions) {
   }
 }
...
@@ -376,6 +422,88 @@ async function executePreparedHeadlessPlan(options: {
   }
 }
...
+} from './pricing.js'
 ...
 test('reports the full active gateway rate range including context tiers', async () => {
   const server = createServer((request, response) => {
@@ -181,3 +189,223 @@ test('pins OpenRouter rate labels to the endpoint tag, not the display name', as
     })
   }
 })
+
...
@@ -159,6 +226,37 @@ export function formatStatuslineCost(props: {
...
@@ -195,6 +293,87 @@ export function sessionCostUsd(
